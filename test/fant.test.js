...
describe.only("Alpha Vault DCA", function () {
  ...
  before(async function () {
    ...
    avdca = await AVDCA.deploy(
      addressConfig[250].WethAddress,
      addressConfig[250].chainlinkFeed,
      addressConfig[250].forwarder,
      addressConfig[250].fee
    );
    ...
    USDC = await ethers.getContractAt(
      "IWETH",
      addressConfig[250].UsdcAddress,
      user
    );
  });

  it("approve USDC token for dca", async function () {
    let tokenAdd = await avdca.addToken(addressConfig[250].UsdcAddress);
    tokenAdd.wait();
  ...
  it("add user funds in dca", async function () {
    user = await ethers.getImpersonatedSigner(
      "0x632085aA7924DF37f64950C58B9Fe5D9fDBA80ef"
    );
    ...
    let depoDCA = await avdca
      ...
      .depositMultipleFunds(
        ...
        100,
        [
          addressConfig[250].LinkAddress,
          addressConfig[250].WethAddress,
          addressConfig[250].WbtcAddress,
        ],
        addressConfig[250].UsdcAddress,
        start,
        start + 300
    ...
    for (let j = 1; j <= 3; j++) {
      ...
      for (let i = 0; i < getUserSchedules.length; i++) {
        ...
        const response = await axios.get(
          `https://fantom.api.0x.org/swap/v1/quote?${qs.stringify(params)}`,
