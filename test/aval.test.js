...
describe.only("Strategy DCA", function () {
  ...
  before(async function () {
    ...
    avdca = await AVDCA.deploy(
      addressConfig[43114].WethAddress,
      addressConfig[43114].chainlinkFeed,
      addressConfig[43114].forwarder,
      addressConfig[43114].fee
    );
    ...
    USDC = await ethers.getContractAt(
      "IWETH",
      addressConfig[43114].UsdcAddress,
      user
    );
  });

  it("approve USDC token for dca", async function () {
    let tokenAdd = await avdca.addToken(addressConfig[43114].UsdcAddress);
    tokenAdd.wait();
  ...
  it("add user funds in dca", async function () {
    ...
    let depoDCA = await avdca
      ...
      .depositMultipleFunds(
        ...
        100,
        [
          addressConfig[43114].LinkAddress,
          addressConfig[43114].WethAddress,
          addressConfig[43114].WbtcAddress,
        ],
        addressConfig[43114].UsdcAddress,
        start,
        start + 300
    ...
    for (let j = 1; j <= 3; j++) {
      ...
      for (let i = 0; i < getUserSchedules.length; i++) {
        ...
        const response = await axios.get(
          `https://avalanche.api.0x.org/swap/v1/quote?${qs.stringify(params)}`,
