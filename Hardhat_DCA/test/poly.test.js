...
describe.only("Alpha Vault DCA", function () {
  ...
  before(async function () {
    ...
    avdca = await AVDCA.deploy(
      addressConfig[137].WethAddress,
      addressConfig[137].chainlinkFeed,
      addressConfig[137].forwarder,
      addressConfig[137].fee
    );
    ...
    USDC = await ethers.getContractAt(
      "IWETH",
      addressConfig[137].UsdcAddress,
      user
    );
  });

  it("approve USDC token for dca", async function () {
    let tokenAdd = await avdca.addToken(addressConfig[137].UsdcAddress);
    tokenAdd.wait();
  ...
  it("add user funds in dca", async function () {
    ...
    let depoDCA = await avdca
      ...
      .depositMultipleFunds(
        ...
        100,
        [
          addressConfig[137].LinkAddress,
          addressConfig[137].WethAddress,
          addressConfig[137].WbtcAddress,
        ],
        addressConfig[137].UsdcAddress,
        start,
        start + 300
