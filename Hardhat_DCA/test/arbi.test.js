...
describe.only("Alpha Vault DCA", function () {
  ...
  before(async function () {
    ...
    avdca = await AVDCA.deploy(
      addressConfig[42161].WethAddress,
      addressConfig[42161].chainlinkFeed,
      addressConfig[42161].forwarder,
      addressConfig[42161].fee
    );
    ...
    USDC = await ethers.getContractAt(
      "IWETH",
      addressConfig[42161].UsdcAddress,
      user
    );
  });

  it("approve USDC token for dca", async function () {
    let tokenAdd = await avdca.addToken(addressConfig[42161].UsdcAddress);
    tokenAdd.wait();
  ...
  it("add user funds in dca", async function () {
    ...
    let depoDCA = await avdca
      ...
      .depositMultipleFunds(
        ...
        100,
        [
          addressConfig[42161].LinkAddress,
          addressConfig[42161].WethAddress,
          addressConfig[42161].WbtcAddress,
        ],
        addressConfig[42161].UsdcAddress,
        start,
        start + 300
