...
describe.only("Alpha Vault DCA", function () {
  ...
  before(async function () {
    deployer= await ethers.getImpersonatedSigner(
        "0x0336E8e0DDD4dE117037028bdC89819F889EC4e3"
      );
    ...
    USDC = await ethers.getContractAt(
      "IWETH",
      addressConfig[5].UsdcAddress,
      user
    );
  });

  it("approve USDC token for dca", async function () {
    let tokenAdd = await avdca.addToken(addressConfig[5].UsdcAddress);
    tokenAdd.wait();
  ...
  it("add user funds in dca", async function () {
    ...
    let depoDCA = await avdca
      ...
      .depositMultipleFunds(
        ...
        100,
        [
          addressConfig[5].LinkAddress,
          addressConfig[5].WethAddress,
          addressConfig[5].WbtcAddress,
        ],
        addressConfig[5].UsdcAddress,
        start,
        start + 300
